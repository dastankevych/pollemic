...
export function SurveyModal({ survey, onCloseAction }: SurveyModalProps) {
  ...
  return (
    <div className="telegram-chat-style h-full flex flex-col">
      {/* Заголовок в стиле Telegram-чата */}
        ...
        <div className="space-y-2 mt-2">
          ...
          <div className="flex justify-center mb-2">
            <div className="bg-[#c5e6b2] text-gray-700 rounded-lg px-3 py-1 text-xs">
              {new Date(survey.created_at).toLocaleDateString("ru-RU", { month: "long", day: "numeric" })}
            </div>
          </div>

          {/* Первое сообщение - название и описание */}
          ...
          {/* Вопросы как отдельные сообщения */}
          {survey.questions.map((question, index) => (
            ...
            >
              ...
              {question.type === "text" && (
                <div></div> // Пустой div вместо строки "Введите ваш ответ..."
              )}
              ...
              {question.type === "multiple" && question.options && (
                <div className="space-y-1">
                  {question.options.map((option, i) => (
                    <div key={i} className="flex items-center space-x-2 bg-gray-100 rounded p-2">
              ...
              <div className="text-xs text-gray-500 mt-2 text-right">{formattedDate.time}</div>
            </div>
          ))}
        </div>
      </div>
